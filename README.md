...
Заглянем в файл views/@conroller_id/_index_js_table.php.
...
класса, перезаписывают параметрамы по умолчанию, и помещаются в переменную "settings". Переменная "settings" часто используется и 
...
Если не понятно, как все устроено, я расскажу философию этой библиотеки.
...
время отправки ajax-запроса и исчезает после завершения этого запроса. В шаблоне _index_js_table.php мы указываем элемент, 
...
index.php в какой-то один, и подгрузить в него _index_js_table.php и _js_plugins.php с обоих crud-ов.
Далее, в шаблонах _index_js_table.php нужно найти методы, которые генерируют url-адреса: ```Url::to(['create'])``` и добавить в адрес
соответствующие контроллеры, например, ```Url::to(['/myController/create'])```.

Помните, что эти crud-ы не мешают друг другу благодаря тому, что они находятся на разных ```$wrapper_selector``` (см. _index_js_table.php),
